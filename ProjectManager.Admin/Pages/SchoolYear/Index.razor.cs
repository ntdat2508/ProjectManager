...
namespace ProjectManager.Admin.Pages.SchoolYear
{
    public class IndexBase : CommonComponentBase
    {
        ...
        public RadzenDataGrid<SchoolYearViewModel> grid;
        public IEnumerable<SchoolYearViewModel> data;
        ...
        public async Task LoadData(LoadDataArgs args)
        {
            ...
            var result = await _schoolYearService.GetAllAsync(request, token);
        ...
        public async Task ShowModal(SchoolYearViewModel data)
        {
            await _dialogService.OpenAsync<SchoolYearModal>(data.Id > 0 ? "Sửa dữ liệu" : "Tạo dữ liệu",
           new Dictionary<string, object>() { { "schoolYearViewModel", data }, { "grid", grid } });
        }

        public async Task ShowModalDelete(long id)
        {
            await _dialogService.OpenAsync<TemplateConfirm>(Constants.Notify,
            new Dictionary<string, object>() { { "table", Constants.FromDelete.SchoolYear }, { "id", id }, { "gridSchoolYear", grid } });
        }

...
namespace ProjectManager.Admin.Pages.TrainingSystem
{
    public class IndexBase : CommonComponentBase
    {
        ...
        public RadzenDataGrid<TrainingSystemViewModel> grid;
        public IEnumerable<TrainingSystemViewModel> data;
        ...
        public async Task LoadData(LoadDataArgs args)
        {
            ...
            var result = await _trainingSystemService.GetAllAsync(request, token);
        ...
        public async Task ShowModal(TrainingSystemViewModel data)
        {
            await _dialogService.OpenAsync<TrainingSystemModal>(data.Id > 0 ? "Sửa dữ liệu" : "Tạo dữ liệu",
             new Dictionary<string, object>() { { "trainingSystemViewModel", data }, { "grid", grid } });
        }

        public async Task ShowModalDelete(long id)
        {
            await _dialogService.OpenAsync<TemplateConfirm>(Constants.Notify,
           new Dictionary<string, object>() { { "table", Constants.FromDelete.TrainingSystem }, { "id", id }, { "gridTrainingSystem", grid } });
        }
